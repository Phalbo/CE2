...
function getInterchangeChordsForChord(key, currentMode, currentChord) {
    const allInterchangeChords = getInterchangeChords(key, currentMode);
    const { root, type } = getChordRootAndType(currentChord);
    const currentRootIndex = NOTE_NAMES.indexOf(root);

    // Prioritize chords with smooth voice leading (shared notes or chromatic movement)
    return allInterchangeChords.filter(interchangeChord => {
        const { root: interchangeRoot, type: interchangeType } = getChordRootAndType(interchangeChord.chord);
        const interchangeRootIndex = NOTE_NAMES.indexOf(interchangeRoot);
        const interval = Math.abs(currentRootIndex - interchangeRootIndex);

        // Favor chords with roots that are a step or a third away
        return interval === 1 || interval === 2 || interval === 3 || interval === 4;
    });
}
