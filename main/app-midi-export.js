...
function handleGenerateSingleTrackChordMidi() {
    ...
    const midiFileNameST = `${title.replace(/[^a-zA-Z0-9_]/g, '_')}_Chords_Pad.mid`;
...
function handleGenerateMelody() {
    ...
    try {
        ...
        if (generatedMelody && generatedMelody.length > 0) {
            const melodyFileName = `${currentMidiData.title.replace(/[^a-zA-Z0-9_]/g, '_')}_Melody.mid`;
            downloadSingleTrackMidi(
                `Melody for ${currentMidiData.title}`, generatedMelody, melodyFileName,
                currentMidiData.bpm, currentMidiData.timeSignatureChanges, 0
            );
        } else { alert("Could not generate 'Fake Inspiration' with the current data. Try again or change parameters."); }
    } catch (e) {
...
function handleGenerateVocalLine() {
    ...
    try {
        ...
        if (vocalLine && vocalLine.length > 0) {
            ...
            downloadSingleTrackMidi(
                `Vocal for ${currentMidiData.title}`, vocalLine, fileName,
                currentMidiData.bpm, currentMidiData.timeSignatureChanges, 0
            );
...
function handleGenerateBassLine() {
    ...
    try {
        ...
        if (bassLine && bassLine.length > 0) {
            ...
            downloadSingleTrackMidi(
                `Bass for ${currentMidiData.title}`, bassLine, fileName,
                currentMidiData.bpm, currentMidiData.timeSignatureChanges, 0
            );
...
function handleGenerateDrumTrack() {
    ...
    try {
        ...
        if (drumEvents && drumEvents.length > 0) {
            ...
            downloadSingleTrackMidi(
                `Drums for ${currentMidiData.title}`, drumEvents, fileName,
